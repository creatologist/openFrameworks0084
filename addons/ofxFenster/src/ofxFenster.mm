...
//-------------------------------------------------------
ofxFenster::ofxFenster(): ofAppGLFWWindow()
{
...
{
    //ofAddListener(ofEvents().setup, baseApp, &ofBaseApp::setup );
...
void ofxFenster::setupOpenGL(int w, int h, int screenMode)
{
    ...
    if(ofxFensterManager::get()->getMainWindow() != NULL)
    {
        shared = ofxFensterManager::get()->getMainWindow()->get()->windowP;
    }
...
void ofxFenster::setWindowTitle(string title)
{
    ...
    glfwSetWindowTitle(windowP, title.c_str());
    //windowP->setWindowTitle( title.c_str() );
}
