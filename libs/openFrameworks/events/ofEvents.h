...
class ofScrollEventArgs : public ofEventArgs, public ofVec2f {
public:
    enum Type{
        Up,
        Down
...
class ofCoreEvents {
  public:
    ...
    ofEvent<ofScrollEventArgs> 	scrolling;
...
template<class ListenerClass>
void ofRegisterScrollEvents(ListenerClass * listener, int prio=OF_EVENT_ORDER_AFTER_APP){
    ofAddListener(ofEvents().scrolling,listener,&ListenerClass::scrolling,prio);
}
